  ...
  public projectDatas: Array<GetAllProjectsResponse> = [];
    ...
    handleSubmitAddProject():void {
      if(this.addProjectForm?.value && this.addProjectForm?.valid){
        const startDate = this.formatDate(this.addProjectForm.value.startDate);
        const endDate = this.formatDate(this.addProjectForm.value.endDate);

        const requestCreateProject: CreateProjectRequest ={
          name: this.addProjectForm.value.name as string,
          description: this.addProjectForm.value.description as string,
          startDate: startDate,
          endDate: endDate,
          status: this.addProjectForm.value.status as string,
          idResponsableUser: Number(this.addProjectForm.value.responsableUser),
          priority: this.addProjectForm.value.priority as string
        };

        this.projectsService.createProject(requestCreateProject)
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: (response) => {
            if (response) {
